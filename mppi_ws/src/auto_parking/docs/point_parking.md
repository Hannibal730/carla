...
2. 열린 변의 실제 양끝을 후보라인 끝점으로 사용한다. Zone Scan의
   `roi_margin=1.0` 영역에서 수집한 점은 후보라인 길이를 확장하지 않는다.
...
yaw는 UTM 평면에서 동쪽이 0 rad이고 반시계 방향이 양수다. ParkingZone1은
폴리곤 중심에서 열린 변을 바라보는 바깥쪽 방향을 사용한다. ParkingZone2는
평행주차 자세를 위해 UTM 동쪽인 `0 rad`로 고정한다. RViz Marker는 UTM을
datum 상대 `map` 좌표로 변환하고 CARLA/ROS Y 반전을 적용해 표시한다.
