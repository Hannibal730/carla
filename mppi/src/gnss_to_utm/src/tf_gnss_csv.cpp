...
class TfGnssCsvNode : public rclcpp::Node
{
public:
    TfGnssCsvNode() : Node("tf_gnss_csv_node")
    {
        // Declare parameters
        ...
        f9r_sub_ = this->create_subscription<geometry_msgs::msg::PointStamped>(
            "/f9r_utm", 10, std::bind(&TfGnssCsvNode::f9r_callback, this, std::placeholders::_1));
        f9p_sub_ = this->create_subscription<geometry_msgs::msg::PointStamped>(
            "/f9p_utm", 10, std::bind(&TfGnssCsvNode::f9p_callback, this, std::placeholders::_1));
        yaw_sub_ = this->create_subscription<std_msgs::msg::Float64>(
            "/azimuth_angle", 10, std::bind(&TfGnssCsvNode::yaw_callback, this, std::placeholders::_1));
        ...
        path_publish_timer_ = this->create_wall_timer(
            std::chrono::milliseconds(50), std::bind(&TfGnssCsvNode::publish_path_callback, this));
    }
...
private:
    ...
    void broadcast_transform(const geometry_msgs::msg::PointStamped::SharedPtr msg, const std::string& child_frame_id)
    {
        ...
        t.header.frame_id = "csv"; // Parent frame for the GNSS sensors
        t.child_frame_id = child_frame_id; // Child frame (f9r or f9p)

        // Translate the GNSS point relative to the CSV origin
...
int main(int argc, char * argv[])
{
    ...
    rclcpp::spin(std::make_shared<TfGnssCsvNode>());
    rclcpp::shutdown();
    return 0;
}