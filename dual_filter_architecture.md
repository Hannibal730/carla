...
**만도맵 (Mando1 / Mando2 / Mando3 / Mando5 / Mando6) 버전:** `--map`에 `Mando1` / `Mando2` / `Mando3` / `Mando5` / `Mando6` 중 하나를 지정한다(사용할 맵이 import 돼 있어야 함, Section 15). `Mando3`은 레퍼런스 경로 기록을 위해 장애물을 제거한 버전(`CustomMap/MandoParking3`)으로 도로 형상·스폰 좌표는 `Mando1`/`Mando2`와 동일하다. `Mando5`(`CustomMap/MandoParking5`, x[-159.8~57.2] y[-88.2~44.1])와 `Mando6`(`CustomMap/MandoParking6`, x[-159.8~71.8] y[-87.9~105.3], 65 roads/5 junctions "full" 버전)은 도로 범위가 더 넓은 별도 맵이라 스폰 좌표가 다르다 — 15.7 E-2로 원하는 위치를 차선에 스냅해 구한다.

```bash
# Mando1/Mando2/Mando3 (동일 차선, 스폰 -93.6/0.0/yaw-90)
cd ~/carla
source .venv/bin/activate
python PythonAPI/util/config.py --map Mando6 \
  && python PythonAPI/examples/manual_control.py --rolename car \
     --filter vehicle.micro.microlino --generation 2 --sync \
...
## 15. 커스텀 맵(RoadRunner) CARLA 적용 매뉴얼 — `Mando1` / `Mando2` / `Mando3` / `Mando5` / `Mando6` 사례
...
>   - `/home/hannibal/carla/CustomMap/MandoParking6/` (RoadRunner 원본 basename은 `Mando_full.*`, import 전 `Mando6.*`로 rename) → 맵 **`Mando6`**. 가장 넓은 "full" 버전(주행 차선 x[-159.8~71.8] y[-87.9~105.3], 65 roads/5 junctions). `.rrdata.xml`·`.geojson`은 import에 **사용하지 않음**
>   - (참고) `MandoParking4`는 RoadRunner 원본 `.xodr`가 손상돼(`</OpenDRIVE>` 태그 3개, XML malformed) import 불가 — 재익스포트 필요
>   - basename(`Mando1`/`Mando2`/`Mando3`/`Mando5`/`Mando6`)이 곧 CARLA 맵 이름. `--map Mando1` … `--map Mando6`로 선택
> - 적용 결과 맵 이름: **`Mando1`**, **`Mando2`**, **`Mando3`**, **`Mando5`**, **`Mando6`** (모두 패키지 **`map_package`** 안에 공존)
...
for name in ('Mando1', 'Mando2', 'Mando3', 'Mando5', 'Mando6'):
...
주차장형). 세 맵은 같은 만도 주차장 기반이라 범위가 거의 동일하나(`Mando3`은 장애물만 제거), 형상을 수정했다면 맵별로 재확인. `Mando5`(x[-159.8 ~ 57.2], y[-88.2 ~ 44.1], 52 roads/4 junctions)와 `Mando6`(x[-159.8 ~ 71.8], y[-87.9 ~ 105.3], 65 roads/5 junctions "full")은 더 넓은 별도 맵이므로 스폰 좌표를 15.7 E-2로 반드시 재산출한다.
...
**만도맵 (Mando1 / Mando2 / Mando3 / Mando5 / Mando6) 버전:** `--map`에 `Mando1` … `Mando6` 지정 (`Mando5`/`Mando6`은 스폰 좌표가 다름 — 아래 E-2로 산출).
...
cp ~/carla/CustomMap/MandoParking1/Mando1.fbx ~/carla/CustomMap/MandoParking1/Mando1.xodr \
   ...
   ~/carla/CustomMap/MandoParking6/Mando6.fbx ~/carla/CustomMap/MandoParking6/Mando6.xodr \
