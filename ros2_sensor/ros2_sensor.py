...
class PythonRos2Publisher:
    def __init__(self, vehicle_id, base_frame, sensors, sensors_config, vehicle=None):
        ...
        self._sensor_qos = QoSProfile(
            ...
            depth=5)
        # robot_localization ekf_node, rviz2 tf2_ros::MessageFilter, and gnss_to_utm C++ nodes
        # all subscribe with RELIABLE QoS (ROS2 default).
        # BEST_EFFORT publisher → RELIABLE subscriber = no DDS connection.
        self._lidar_qos = QoSProfile(
            reliability=ReliabilityPolicy.RELIABLE,
            history=HistoryPolicy.KEEP_LAST,
            depth=5)
        self._ekf_qos = QoSProfile(
            reliability=ReliabilityPolicy.RELIABLE,
            history=HistoryPolicy.KEEP_LAST,
            depth=10)
    ...
    def _start_publishers(self, vehicle_id, sensors, sensors_config):
        for sensor, sensor_config in zip(sensors, sensors_config):
            ...
            if sensor_type.startswith("sensor.camera."):
                ...
                publisher = self.node.create_publisher(
                    self.PointCloud2, "{}/point_cloud".format(topic_prefix), self._lidar_qos)
                sensor.listen(
                    lambda data, pub=publisher, frame_id=sensor_id:
                ...
                publisher = self.node.create_publisher(
                    self.Imu, "{}/data".format(topic_prefix), self._ekf_qos)
                sensor.listen(
                    lambda data, pub=publisher, frame_id=sensor_id:
